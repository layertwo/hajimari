...
{#each group.apps || [] as app (app.name)}
  <div class="apps_item" in:fade|local={{ duration: 300 }} animate:flip={{ duration: 300 }}>
    <div class="apps_icon">
      <a
        href={app.url}
      ...
      {#if app.replicas.total > 0 && showStatus}
        <hr
          class="app_status"
          style="background-image: linear-gradient(to right, var(--color-text-acc) 0 {app.replicas
            .pctReady}%, currentcolor {app.replicas.pctReady}% 100%);"
        />
      {/if}
    </div>
    <div class="apps_text">
