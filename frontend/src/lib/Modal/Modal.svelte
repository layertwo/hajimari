...
<div class="modal" role="dialog" aria-modal="true" use:portal={"body"} hidden transition:slide>
  <header>
    <h1>Settings</h1>
    <div>
      <a href="#close" title="Close" class="modal-close" on:click|preventDefault={close}>
        <Icon icon="mdi:close" />
      </a>
      <a href="#help" title="Help" class="modal-help" on:click|preventDefault={help}>
    ...
    <section id="providers" transition:scale>
      ...
      <table>
        ...
        {#each settings.searchProviders as provider}
          <tr>
            <td
              ><a href={provider.url} target="_blank" rel="external noopener noreferrer"
  ...
  <footer>
    ...
    >
    <a href="https://discord.gg/HWGZSWJsA8" target="_blank" rel="external noopener noreferrer"
      ><Icon icon="mdi:discord" /></a
    >
    <a href="https://icon-sets.iconify.design" target="_blank" rel="external noopener noreferrer"
      ><Icon icon="mdi:material-design" /></a
    >
    <button on:click={handleSave} type="button" id="save-config" class="save-button">Save</button>
  </footer>
</div>
